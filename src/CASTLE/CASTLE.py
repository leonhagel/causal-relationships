...
class CASTLE(object):
    ...
    #             standardize = True,  reg_lambda=None, reg_beta=None, DAG_min = 0.5, sigmoid_output=[]):    
    def __init__(self, lr=0.001, batch_size=32, num_outputs=1, w_threshold=0.3, n_hidden=32, hidden_layers=2, ckpt_file='tmp.ckpt', reg_lambda=1, reg_beta=1, sigmoid_output=[], random_state=123, loss_function="mse"):
        ...
        self.out_layer = {}
        self.Out_0 = []
        self.mask = {} # Mask removes the feature i from the network that is tasked to construct feature i
        self.activation = tf.nn.relu
        self.W_0 = []
        
    def build(self, X, num_train=None, num_inputs = None):
        self.num_train = num_train if num_train is not None else X.shape[0]
        self.num_inputs = num_inputs if num_inputs is not None else X.shape[1]
        ...
        for i in range(self.num_inputs):
            self.weights['w_h0_'+str(i)] = tf.Variable(tf.random_normal([self.num_inputs, self.n_hidden], seed = self.seed)*0.01)
            self.weights['out_'+str(i)] = tf.Variable(tf.random_normal([self.n_hidden, self.num_outputs], seed = self.seed))
            
        for i in range(self.num_inputs):
            self.biases['b_h0_'+str(i)] = tf.Variable(tf.random_normal([self.n_hidden], seed = self.seed)*0.01)
            self.biases['out_'+str(i)] = tf.Variable(tf.random_normal([self.num_outputs], seed = self.seed))
        ...
        for i in range(self.num_inputs):
            ...
            #### binary target = sigmoid activation
            if i in self.sigmoid_output: 
                self.out_layer['nn_'+str(i)] = tf.nn.sigmoid(tf.matmul(self.hidden_h1['nn_'+str(i)], self.weights['out_'+str(i)]) + self.biases['out_'+str(i)])
            else:
                ####
        ...
        supervised_loss = {
            "mse": tf.reduce_mean(tf.reduce_sum(tf.square(self.out_layer['nn_0'] - self.y),axis=1),axis=0),
            "bce": bce(self.y, self.out_layer['nn_0'])
        }
        ...
        self.supervised_loss = supervised_loss[self.loss_function]
        ...
        self.average_loss = 0.5 / self.num_train * tf.reduce_sum(tf.square(self.R))
    ...
    def fit(self, X, y, X_val, y_val, num_nodes=None, X_test=None, y_test=None, verbose=1): ## added verbose, changed test to optional
        ...
        for step in range(1, self.max_steps):
            ...
            print("Step " + str(step) + ", Loss= " + "{:.4f}".format(loss)," h_value:", h_value) if verbose >=1 else None
            ...
            if step >= self.saves:
                try:
                    if val_loss < best:
                        ...
                        print("Saving model") if verbose >=1 else None
                        self.count = 0
                    else:
                        self.count += 1
                except:
            ...
            if self.count > self.patience:
                print("Early stopping") if verbose >=1 else None
                break
    ...
    def pred(self, X, add_target_dummy=1e6): # new: add_target_dummy=True, features have to be located in X[:, 1:], first column = target, but not relevant for prediction
        ###
        if add_target_dummy != False: 
            X = np.hstack([np.full((X.shape[0], 1), add_target_dummy), X])
        ###
    ...
    def pred_W(self, X, y):
        ...
        ### new: append y to X if not included
        except ValueError:
            X = np.hstack([y, X])
            W_est = self.sess.run(self.W, feed_dict={self.X: X, self.y: y, self.keep_prob : 1, self.rho:np.array([[1.0]]), self.alpha:np.array([[0.0]]), self.is_train : False, self.noise:0})
        ###
